...
struct tuple{n}
{{
#define OPEQ_TUPLE(OP) inline constexpr tuple{n} &operator OP(const tuple{n} &rhs) {{ {' '.join(f'{L[i]} OP rhs.{L[i]};' for i in range(1,n+1))} return *this; }}
#define OP_BINARY(OP) friend inline constexpr auto operator OP(const tuple{n} &lhs, const tuple{n} &rhs) \\
    {{ return make_tuple{n}({', '.join(f'lhs.{L[i]} OP rhs.{L[i]}' for i in range(1,n+1))}); }}
#define OP_EQ_BIN(OP) OPEQ_TUPLE(OP##=) OP_BINARY(OP)
#define OP_UNARY(OP) friend inline constexpr auto operator OP(const tuple{n} &tup) {{ return make_tuple{n}({', '.join(f'OP tup.{L[i]}' for i in range(1,n+1))}); }}
#define OP_PRE(OP) inline constexpr tuple{n} &operator OP() {{ {' '.join(f'OP {L[i]};' for i in range(1,n+1))} return *this; }}
#define OP_POST(OP) inline constexpr tuple{n} operator OP(int) {{ tuple{n} ret = *this; OP(*this); return ret; }}
    ...
    inline constexpr bool operator==(const tuple{n} &tup) const {{ return {' && '.join(f'{L[i]} == tup.{L[i]}' for i in range(1,n+1))}; }}
    inline constexpr auto operator<=>(const tuple{n} &tup) const
    {{
        auto cmp = (a <=> tup.a);
        if (cmp != 0) return cmp;{'' if n == 2 else '\n        '
                                  + '\n        '.join(f'if ((cmp = ({L[i]} <=> tup.{L[i]})) != 0) return cmp;' for i in range(2,n))}
        return {L[n]} <=> tup.{L[n]};
    }}
#undef OPEQ_TUPLE
#undef OPEQ_OTHER
